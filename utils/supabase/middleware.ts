...
export async function updateSession(request: NextRequest) {
  ...
  // 1. Mandatory Onboarding Check (BE Kit form not yet submitted)
  ...
  // 3. Role-Based Redirection for /portal
  ...
  // 4. Root Redirection for logged-in users
  if (pathname === '/') {
    if (user) {
      const role = (user.user_metadata?.role || 'client').toLowerCase()
      if (!user.user_metadata?.onboarding_completed && role !== 'super admin' && role !== 'admin') {
        url.pathname = '/onboarding/be-kit'
